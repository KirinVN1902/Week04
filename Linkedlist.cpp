...
bool removeHead(List *&L)
{
    ...
    else if(L->p_head == L->p_tail && L->p_head != nullptr){
        NODE *p = L->p_head;
        L->p_head = nullptr;
        L->p_tail = nullptr;
        delete p;
...
void removeBefore(List *&L, int val)
{
    ...
    if(L->p_head != nullptr && L->p_tail == L->p_head){
        return;
    }
    ...
    NODE *searchVal = cur->p_next;
    NODE *pre = L->p_head;
    while(searchVal != nullptr){
        if(searchVal->key == val){
            pre->p_next = searchVal;
            delete cur;
            return;
        }
        pre = pre->p_next;
        searchVal = searchVal->p_next;
        cur = cur->p_next;
    }
    return;
}
...
void removeAfter(List *&L, int val)
{
    ...
    NODE *searchVal = L->p_head;
    while(searchVal != nullptr){
        if(searchVal->key == val){
            NODE *p = searchVal->p_next;
                if(p == nullptr){
                    return;
                }
            searchVal->p_next = p->p_next;
            if(p == L->p_tail){
                L->p_tail = searchVal;
            }
            delete p;
...
{   
    ...
    NODE *prev = L->p_head;
    while(prev->p_next != searchVal){
        prev = prev->p_next;
    }
    p->p_next = prev->p_next;
    prev->p_next = p;
    return true;
}
...
void removeDuplicate(List *&L)
{
    ...
    while(cur != nullptr){
        ...
        while(runner->p_next != nullptr){
            if(runner->p_next->key == cur->key){
                NODE *p = runner->p_next;
